...
const ShootWaste: React.FC = () => {
    ...
    const handleLoad = () => {
        console.log('Shoot game iframe loaded successfully');
        setIsLoading(false);
        setError(null);
    };

    const handleError = () => {
        console.error('Shoot game iframe failed to load');
        setError('Failed to load the Shoot Game. Please try refreshing the page.');
        setIsLoading(false);
    };
    ...
    return (
        <div className="w-full h-[calc((100vh-200px)*3/4)] min-h-[450px] md:h-[calc(100vh-200px)] md:min-h-[600px] max-md:w-[110%] max-md:h-[calc((100vh-200px)*3/4*1.1)] max-md:-ml-[5%]">
            {isLoading && (
                <div className="flex items-center justify-center h-full bg-gray-100 rounded-lg">
                    <div className="text-center">
                        ...
                        <p className="text-gray-600">Loading Shoot Game...</p>
            ...
            {error && (
                <div className="flex items-center justify-center h-full bg-red-50 rounded-lg">
                    <div className="text-center p-6">
                        ...
                        <p className="text-sm text-red-600 mt-2">Target URL: /shootwaste.html?lang={language}</p>
                        <button 
            ...
            <iframe
                ...
                className={`w-full h-full border-0 rounded-lg ${isLoading || error ? 'hidden' : ''}`}
